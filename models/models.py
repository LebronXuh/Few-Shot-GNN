...
class EmbeddingDbh(nn.Module):
    ''' In this network the input image is supposed to be 84x84 '''

    def __init__(self, args, emb_size):
        super(EmbeddingDbh, self).__init__()
        self.emb_size = emb_size
        self.ndf = 64
        self.args = args

        # Conv2d(self, in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias, padding_mode)
        # Input 84x84x3
        self.conv1 = nn.Conv2d(3, self.ndf, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(self.ndf)
...
class MetricNN(nn.Module):
    def __init__(self, args, emb_size):
        ...
        if self.metric_network == 'gnn_iclr_nl':
            ...
            if self.args.dataset == 'mini_imagenet' or 'dbh':
...
def create_models(args):
    ...
    elif 'dbh' == args.dataset:
        enc_nn = EmbeddingDbh(args, 128)
    else:
